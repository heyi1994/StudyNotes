# 学习笔记

在线访问：[https://melrose1994.com/docs/](https://melrose1994.com/docs/)

基于 [MkDocs](https://www.mkdocs.org/) + [Material 主题](https://squidfunk.github.io/mkdocs-material/) 构建的个人技术学习笔记，涵盖全栈开发、移动端、DevOps、逆向工程等领域。

## 内容

- **编程语言** — Rust（含 std 系列：错误处理、泛型/Trait、迭代器与闭包、生命周期、OnceCell/Lazy、智能指针、集合类型、并发原语、async/await）、TypeScript、垃圾回收算法
- **前端** — React、Next.js、Redux、Zustand、Tailwind CSS、Sass、JavaScript ES6~ES2025、现代 Web 平台新特性
- **后端** — Docker、Axum、sqlx
- **移动端** — Flutter（含 Monorepo 实践、Passkey 通行密钥）、Android Compose、Android 17 行为变更、AOSP 构建 Rust 模块、React Native
- **CI/CD** — GitHub Actions、GitLab CI/CD、Jenkins、Fastlane、Webhook
- **逆向工程** — Frida、LSPosed、JADX
- **游戏开发** — Godot、GDScript、Input 输入系统、Physics & Signals、Parallax2D、常用 2D 节点、Animation 动画系统、Layout UI 布局、TileMapLayer 瓦片地图、2D 导航系统、.tres 资源文件
- **AI** — GitNexus、OpenAI API 协议、Agent 开发（TypeScript + Claude）

## 本地运行

```bash
pip install -r requirements.txt
mkdocs serve
```

浏览器访问 http://127.0.0.1:8000

## 构建

```bash
mkdocs build
```

静态文件输出到 `site/` 目录。

